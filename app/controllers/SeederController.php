...
class SeederController extends Controller {
    public function run() {
        ...
        $db = (new Model())->query("SELECT 1")->queryString ? (new Model()) : null;
        if (!$db) die("Error al conectar DB.");
        
        $pdo = (new PDO(
            "mysql:host=localhost;port=3306;dbname=barber_hub;charset=utf8mb4", 
            "root", 
            "", 
        ...
        $imagenes = [
            BASE_URL . '/assets/img/hero_barber_1780262216201.png',
            BASE_URL . '/assets/img/shop_modern_1780262229137.png',
            BASE_URL . '/assets/img/shop_vintage_1780262306705.png'
        ];
        ...
        // 3. PROPIETARIOS, MODERADORES Y BARBEROS Y SERVICIOS POR BARBERIA
        ...
        for ($i = 0; $i < 2500; $i++) {
            ...
            // Buscar un servicio de esa barbería
            $b_id = $barb['b_id'];
            $srvs = array_filter($servicios_data, function($s) use ($b_id) { return $s['b_id'] == $b_id; });
            if (empty($srvs)) continue;
            $srv = $srvs[array_rand($srvs)];
            
            $fecha = date('Y-m-d', strtotime(($i < 2000 ? "-" . mt_rand(1, 365) : "+" . mt_rand(0, 30)) . " days"));
            $estado = $i < 2000 ? ((mt_rand(1,10) > 1) ? 'Finalizada' : 'Cancelada') : ((mt_rand(1,10) > 2) ? 'Confirmada' : 'Pendiente');
            ...
            if ($estado == 'Finalizada') {
                ...
                if (mt_rand(1,10) <= 6) {
                    $comentarios = ['Excelente servicio y trato.', 'Me encantó.', 'Buen ambiente.', 'Rápido y eficiente.'];
